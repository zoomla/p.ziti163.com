...
namespace ZLCommon.Helper
{
    public class FileUDHelper
    {
        ...
        /// <param name="vpath">保存,带文件名</param>
        /// <param name="flag">标记,用于记录状态(Session)</param>
        public static void DownFile(string url, string ppath, ProgMod model)
        {
            //string ppath = function.VToP(vpath);
            //if (SafeSC.FileNameCheck(ppath)) { function.WriteErrMsg(vpath + "文件不允许下载"); }
