...
namespace ZLCommon.Helper
{
    public class FileSystemObject
    {
        ...
        public static DataTable GetDirectoryInfos(string dir, FsoMethod method)
        {
            ...
            if (method != FsoMethod.Folder)
            {
                for (num = 0; num < getFiles(dir).Length; num++)
                {
                    ...
                    row["ExSize"] = ((info2.Length / 1024) + (info2.Length % 1024 > 1 ? 1 : 0)).ToString() + "KB";
                    row["ExName"] = info2.Extension;
                    row["ExType"] = "";
                    row["FullPath"] = info2.FullName;
                    table.Rows.Add(row);
                }
            }
        ...
        {
            string ppath = IOHelper.MapPath(vpath);
