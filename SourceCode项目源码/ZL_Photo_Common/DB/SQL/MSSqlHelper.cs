...
namespace ZoomLa.SQLDAL.SQL
{
    public class MSSqlHelper : SqlBase
    {
        ...
        public override DataTable SelPage(PageSetting config)
        {
            ...
            {
                config.sql = "SELECT TOP " + config.psize + config.fields + " FROM " + config.t1 + " A WHERE " + config.pk + " Not In(Select Top " + config.cursize + " " + config.pk + " FROM " + config.t1 + " A WHERE " + config.where + config.order + ") And " + config.where + config.order;
        ...
        {
            string sql = "CREATE TABLE [dbo].[" + table + "] ({0})";
        ...
        {
            //去除约束
            DataTable constraintDT = ExecuteTable(new SqlModel("SELECT b.[name] FROM syscolumns a LEFT JOIN sysobjects b ON a.cdefault=b.id WHERE a.id=object_id('" + table + "') and a.name='" + field + "'", null));
            foreach (DataRow dr in constraintDT.Rows)
            {
                ExecuteNonQuery(new SqlModel("ALTER TABLE " + table + " DROP CONSTRAINT " + dr["name"], null));
            }
            ExecuteNonQuery(new SqlModel() { sql = "ALTER TABLE " + table + " DROP COLUMN " + field });
            return true;
        }
